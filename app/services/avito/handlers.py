...
class AvitoMessageHandlers:
    ...
    async def handle_text_message(chat_id: str, text: str, author_id: str) -> str:
        """Обрабатывает текстовое сообщение через FSM диалога."""
        logger.info(
            "Avito: сообщение в чате %s от %s: %s",
            chat_id,
            author_id,
            text[:80],
        )

        from app.services.avito.conversation_manager import conversation_manager

        answer = await conversation_manager.handle_message(chat_id, text)
